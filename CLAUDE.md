...
- **`PPDocLayoutSLayoutAnalyzer`** wraps PP-DocLayout-S, a PicoDet/GFL export (23 classes, `ProvidesReadingOrder=false`, ~4.7 MB). Pipeline: bilinear-resize the rasterized page to 480×480 float NCHW with ImageNet normalisation → ONNX (`image` + `scale_factor` inputs, **no `im_shape`**) → `[M,6]` detections `[classId, confidence, xmin, ymin, xmax, ymax]` + scalar `num_dets` (NMS baked in at score_threshold=0.3) → confidence filter → NMS. Class table + role mapping in `PPDocLayoutSRoles`. **Note the InputSize/ModelInputSize split**: the caller rasterises at 1920 longest-edge (advertised via `Capabilities.InputSize`) — going straight to 480 loses bibliography rows and small text on academic content, so the analyzer downsizes the 1920-edge pixmap to 480×480 internally before inference. This was a load-bearing lesson from the Python `raildla` prototype.
...
All three take an optional `LayoutDetectionTuning` constructor argument (Core) overriding the detection thresholds otherwise read from `LayoutConstants` — confidence, NMS IoU, minimum detection size. Its sibling `LineDetectionTuning` carries the raster thresholds of the pixel-projection line fallback and is accepted by `LineDetector.DetectLines` / `BlockPostProcessor.PostProcess` / `AnalysisWorker` / `DocumentController.InitializeWorker`. Two records, not one, because each seam ignores the other's values — a merged record would let a caller set a confidence threshold on the worker and have it silently do nothing. Both `.Default`s are byte-for-byte the old constant behaviour.
...
Activates above `CoreSettings.RailZoomThreshold` when analysis is available. Locks to detected text blocks, advances line-by-line with cubic ease-out snap. Sub-features: auto-scroll, jump mode, line focus blur, line highlight, named bookmarks, table-row reading + cell navigation. Free pan (Ctrl+drag) temporarily exits rail mode for unrestricted pan/zoom, snapping back on Ctrl release. Rail state lives on each `Viewport` (`RailNav`), so multiple viewports of one document navigate independently.

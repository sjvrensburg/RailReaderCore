...
All three take an optional `LayoutTuning` constructor argument (Core) that overrides the model-agnostic thresholds otherwise read from `LayoutConstants` — confidence, NMS IoU, minimum detection size; the raster thresholds in the same record are accepted by `LineDetector.DetectLines` / `BlockPostProcessor.PostProcess` / `AnalysisWorker` / `DocumentController.InitializeWorker`. `LayoutTuning.Default` is byte-for-byte the old constant behaviour.

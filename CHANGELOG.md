...
- **Auto-scroll timing now follows the frame clock rather than a timer read during each tick.**
  `AutoScrollStateMachine` placed the camera using a `Stopwatch` it read inside `Tick` and ignored
  the host's `dt`. The UI-thread callback that runs the tick starts at a slightly different point
  in each frame, while frames are displayed at a steady rate. As a result, each displayed step was
